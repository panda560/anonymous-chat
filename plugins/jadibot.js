...
let handler = async (m, { conn, args, usedPrefix, command, isOwner }) => {
  ...
  if ((args[0] && args[0] == 'plz') || global.conn.user.jid == conn.user.jid) {
    ...
    conn.browserDescription = ['anonymous-chat', '', '5.0']
    ...
    conn.connect().then(async ({ user }) => {
      ...
      if (auth) return
      await parent.sendMessage(user.jid, `Kamu bisa login tanpa qr dengan pesan dibawah ini. untuk mendapatkan kode lengkapnya, silahkan kirim *${usedPrefix}getcode* untuk mendapatkan kode yang akurat`, 'extendedTextMessage')
      parent.sendMessage(user.jid, `${usedPrefix + command} ${Buffer.from(JSON.stringify(conn.base64EncodedAuthInfo())).toString('base64')}`, 'extendedTextMessage')
    })
    setTimeout(() => {
      if (conn.user) return
      conn.close()
      ...
      setTimeout(async () => {
        try {
          ...
          if (conn.user && conn.user.jid)
            parent.sendMessage(conn.user.jid, `Koneksi terputus...`, 'extendedTextMessage')
          let i = global.conns.indexOf(conn)
          if (i < 0) return
          delete global.conns[i]
          global.conns.splice(i, 1)
...
handler.command = /^(jadibot|scan)$/i
