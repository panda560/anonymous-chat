...
`.trim()
    ...
    async sendContactArray(jid, array, quoted, options) {
      let list = []
      for (let i of array.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net')) {
        let name = this.getName(i)
        list.push({
          "displayName": name,
      ...
      return await this.sendMessage(jid, {
        ...
        "contacts": list
      }, MessageType.contactsArray, { quoted, ...options })
    }
 ...
 */
    ...
    */
    ...
    async sendFile(jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) {
      ...
      return await this.sendMessage(jid, file, mtype, { ...opt, ...options, contextInfo: { mentionedJid: this.parseMention(caption) } })
    }
